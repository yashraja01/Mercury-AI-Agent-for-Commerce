...
const rupees = (p: number): string => `Rs ${(p / 100).toFixed(2)}`;
...
const feed = await tool<{ count: number; products: { id: string; title: string }[] }>(
  "search_catalog",
  { merchant_id: "mch_quick", query: "rice" },
);
console.log(`  search     "rice" -> ${feed.products.map((p) => p.id).join(", ")}`);

head("2. Budget — what the human actually authorised");
const budget = await tool<{
  remaining_paise: number;
  max_per_txn_paise: number;
  requires_human_approval_above_paise: number;
  human_present: boolean;
}>("check_budget", { mandate_id: "mnd_household_weekly" });
console.log(`  remaining  ${rupees(budget.remaining_paise)}`);
console.log(`  per-txn    ${rupees(budget.max_per_txn_paise)}`);
console.log(`  step-up    above ${rupees(budget.requires_human_approval_above_paise)}`);
...
interface Quote {
  ...
  reply: string;
  rounds: number;
  cart?: { total_paise: number; order_id: string; intent_token_id: string };
...
const q = await tool<Quote>("request_quote", {
  ...
  message: "two bags of rice and a pack of tea, and I want it all for 300 rupees",
});
console.log(`  buyer      "…and I want it all for 300 rupees"`);
console.log(`  outcome    ${q.outcome} in ${q.rounds} round(s)`);
console.log(`  merchant   ${q.reply}`);
console.log(`  charged    ${q.cart === undefined ? "nothing" : rupees(q.cart.total_paise)}`);
console.log(`             the buyer asked for Rs 300.00 and the gate priced it anyway`);
...
const denied = await tool<Quote>("request_quote", {
  merchant_id: "mch_quick",
  mandate_id: "mnd_household_weekly",
  message: "eight bags of rice and eight bags of atta",
});
