...
function isPaise(v: unknown): boolean {
...
async function feed(merchantId: string): Promise<void> {
   ...
   */
  check(doc, "declares a currency at the document level", f.currency === "INR", String(f.currency));
  check(doc, "declares its minor unit", f.minor_unit === "paise", String(f.minor_unit));
  ...
  const badPrice = products.find((p) => !isPaise(p.price?.amount));
  check(doc, "every price is an integer minor unit", badPrice === undefined,
    badPrice === undefined ? "all integer paise" : `${String(badPrice.id)} -> ${String(badPrice.price?.amount)}`);

  const badUnit = products.find(
    (p) => p.price?.currency !== "INR" || p.price?.minor_unit !== "paise",
  );
  ...
  const badMoq = products.find((p) => !isPaise(p.min_order_quantity) || Number(p.min_order_quantity) < 1);
