    ...
    <div className="settle-in ml-[80px] border border-rule bg-ink-sunk">
      <div className="flex items-baseline justify-between border-b border-rule px-3 py-1.5">
        ...
        <span className="figures text-[11px] text-paper-faint">
          agent quotes {rupees(event.quoted_paise)}
        </span>
      </div>
      ...
      <table className="w-full text-[12px]">
        <tbody>
          {event.lines.map((l) => {
            const off = l.list_paise > 0 ? 1 - l.unit_paise / l.list_paise : 0;
            return (
    ...
    case "order":
      return (
        <Marker tone="brass" label="order">
          {event.order_id} for {rupees(event.amount_paise)} — the amount Dwaar computed
        </Marker>
      );
    case "stepup":
      return (
