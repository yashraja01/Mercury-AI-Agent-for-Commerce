  ...
  return (
    ...
    >
      ...
      <span className={spent ? "text-brass" : "text-paper-dim"}>
        {bench.txn_count}/{bench.max_txn_count} debits used &middot; {rupees(bench.remaining_paise)}{" "}
        left &middot;{" "}
...
export function ChaosPanel({ onSettled }: { onSettled: () => void | Promise<void> }) {
   ...
   * the Sakshi chain is the artifact this whole product asks to be trusted on;
    ...
    <section className="panel flex min-h-0 flex-1 flex-col">
      ...
      <div className="border-b border-rule px-4 py-3">
        <p className="max-w-[78ch] text-[13px] leading-relaxed text-paper-dim">
          Seven engineered failures. Each one injects a real fault — a lying agent, a declining
          rail, a forged signature, a replayed token — and then checks that the recovery happened
          and that Sakshi recorded it. Nothing here is simulated at the boundary: the webhook rows
          arrive as HTTP requests to this app&apos;s own Razorpay endpoint.
        </p>
        <div className="mt-3">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          <span className="text-[12px] text-paper-faint">
            F1–F7 in order. Each row runs against the live gate, ledger and rail.
          </span>
        </div>
      </div>
