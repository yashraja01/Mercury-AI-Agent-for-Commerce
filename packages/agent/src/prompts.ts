 ...
 * be hashed and pinned -- `promptHash()` is what goes into the Sakshi ledger, so
 ...
 * the buyer's message, Dwaar's verdicts) goes into `messages` after the cache
...
/** Stable identity for a prompt body, for the `llm.input_hash` field in Sakshi. */
