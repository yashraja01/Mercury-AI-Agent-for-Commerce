...
/** What Dwaar said about an offer, flattened for the agent to read. */
export interface GateFeedback {
  outcome: "ALLOW" | "ALLOW_WITH_STEPUP" | "DENY";
  rule_ids: RuleId[];
  /** Deterministic rule messages. Never model-generated. */
  messages: string[];
  /** Dwaar's own total. Present when the offer was allowed. */
...
export interface NegotiationResult {
  ...
  reply: string;
  /** Every offer made this turn, in order. More than one means Dwaar pushed back. */
...
export interface NegotiatorContext {
  ...
  /** sku -> item, for this merchant. Carries cost_paise, which is never exposed. */
