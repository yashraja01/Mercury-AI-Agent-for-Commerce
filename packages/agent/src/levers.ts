import type { CatalogItem, Lever, MerchantProfile, Paise } from "@mercury/core";
import { mulP, paise, sumP } from "@mercury/core";
 ...
 * here decides whether the basket is allowed. Dwaar still reprices every line
...
export function bulkTierUnit(
  ...
  tiers: readonly BulkTier[] = DEFAULT_TIERS,
): { unit: Paise; tier: BulkTier; clamped: boolean } {
  const tier = tierFor(qty, tiers);
  const floor = lowestLegalUnit(item, profile);
  const wanted = item.list_paise - Math.floor((item.list_paise * tier.discount_bps) / 10_000);
  const unit = Math.max(wanted, floor);
  return { unit: paise(unit), tier, clamped: unit > wanted };
}
...
export function bundleAddOn(
  ...
  const basketTotal = sumP(lines.map((l) => mulP(l.unit_paise, l.qty)));
  if (basketTotal <= 0) return undefined;

  const inCart = new Set(lines.map((l) => l.sku));
  const anchorSku = [...lines].sort(
    (a, b) => mulP(b.unit_paise, b.qty) - mulP(a.unit_paise, a.qty),
  )[0]?.sku;
  ...
  let best: { item: CatalogItem; unit: Paise } | undefined;
  for (const item of [...catalog.values()].sort((a, b) => (a.sku < b.sku ? -1 : 1))) {
    ...
    const floor = lowestLegalUnit(item, profile);
    const wanted = item.list_paise - Math.floor((item.list_paise * sweetener) / 10_000);
    const unit = paise(Math.max(wanted, floor));
  ...
  return {
    ...
    qty: best.item.moq,
    unit_paise: best.unit,
    list_paise: best.item.list_paise,
 ...
 * another category, because a buyer who asked for rice does not want soap.
 */
export function substituteFor(
  ...
  for (const item of catalog.values()) {
    ...
    if (
      best === undefined ||
      Math.abs(item.list_paise - wanted.list_paise) < Math.abs(best.list_paise - wanted.list_paise)
    ) {
      best = item;
    }
  }
  ...
  const unit = paise(Math.max(best.list_paise, lowestLegalUnit(best, profile)));
  return {
    from_sku: sku,
    to_sku: best.sku,
    to_title: best.title,
 ...
 * of whether the Revenue Agent earned its place, and it is recorded in Sakshi
