...
import type { EntryBody, EntryInput, EventType, SakshiEntry, VerifyResult } from "./types.js";
...
export class Sakshi {
  ...
  static open(path: string): Sakshi {
    const db = new DatabaseSync(path);
    db.exec("PRAGMA journal_mode = WAL");
    db.exec("PRAGMA foreign_keys = ON");
    return new Sakshi(db);
  }
  ...
  #migrate(): void {
    this.#db.exec(`
      CREATE TABLE IF NOT EXISTS sakshi (
      ...
      CREATE INDEX IF NOT EXISTS sakshi_event_type ON sakshi(event_type);
    `);
  }
   ...
   */
  append(input: EntryInput): SakshiEntry {
    this.#db.exec("BEGIN IMMEDIATE");
    try {
      const tipRow = this.#db
        .prepare("SELECT seq, hash FROM sakshi ORDER BY seq DESC LIMIT 1")
      ...
      this.#db
        .prepare(
          "INSERT INTO sakshi (seq, ts, event_type, body, prev_hash, hash) VALUES (?, ?, ?, ?, ?, ?)",
        )
  ...
  read(opts: { from?: number; limit?: number } = {}): SakshiEntry[] {
    const from = opts.from ?? 1;
    const limit = opts.limit ?? 10_000;
    const rows = this.#db
      .prepare(
        "SELECT seq, ts, event_type, body, prev_hash, hash FROM sakshi WHERE seq >= ? ORDER BY seq ASC LIMIT ?",
      )
  ...
  byEventType(t: EventType): SakshiEntry[] {
    const rows = this.#db
      .prepare(
        "SELECT seq, ts, event_type, body, prev_hash, hash FROM sakshi WHERE event_type = ? ORDER BY seq ASC",
      )
  ...
  count(): number {
    const r = this.#db.prepare("SELECT COUNT(*) AS n FROM sakshi").get() as { n: number };
    return r.n;
  }

  tipHash(): string {
    const r = this.#db.prepare("SELECT hash FROM sakshi ORDER BY seq DESC LIMIT 1").get() as
      | { hash: string }
      | undefined;
  ...
  verify(): VerifyResult {
    const rows = this.#db
      .prepare("SELECT seq, ts, event_type, body, prev_hash, hash FROM sakshi ORDER BY seq ASC")
