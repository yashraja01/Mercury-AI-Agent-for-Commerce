 ...
 * Auto-repair.
 *
 * When the Revenue Agent proposes a price Dwaar rejects, we do not simply fail
...
export function repairProposal(
  ...
  const lines = proposal.lines.map((pl) => {
    ...
    if (pl.offer_unit_paise >= lowest) return pl;

    changed = true;
    adjustments.push(
      `${pl.sku}: ${formatINR(pl.offer_unit_paise)} -> ${formatINR(lowest)} per ${item.unit} ` +
        `(lowest price permitted by the margin floor and discount ceiling)`,
    );
    return { ...pl, offer_unit_paise: lowest };
  });

  const recomputedTotal = sumP(lines.map((l) => mulP(l.offer_unit_paise, l.qty)));

  return {
    proposal: { ...proposal, lines, quoted_total_paise: recomputedTotal },
    adjustments,
    changed,
  };
}
